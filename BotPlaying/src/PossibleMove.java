...
public class PossibleMove {
    ...
    public void setFitness() {
        ...
        fitness = (scoreGained*1000000) - (heightOfField*20000) - (numberOfGaps*1000) - (gaps*10000);
    }
    public static int gapsInField(int[][]field, int heightOfField) {
        int gaps=0;
        for(int i= 0; i<heightOfField; i++){
            for(int j = 0; j<field.length; j++){
                if(field[j][field[0].length-1-i]<0){
                    gaps++;
                }
            }
        }
    ...
    /**
     * Calculates the height of the field after a given move
     * @param field
